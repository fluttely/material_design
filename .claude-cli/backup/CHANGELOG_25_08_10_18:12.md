...
- **New Adaptive Design System**: A comprehensive adaptive design system has been introduced in `lib/src/foundations/adaptive.dart`. This new system provides a robust set of tools for building responsive and adaptive user interfaces that are compliant with Material Design 3.
  - **Window Size Classes**: `WindowSizeClass` enum for classifying window sizes into compact, medium, and expanded.
  - **Canonical Layouts**: `CanonicalLayout` enum for implementing standard layout patterns like list-detail and supporting pane.
  - **Adaptive Configuration**: `AdaptiveConfig` class for managing adaptive layouts based on screen size.
  - **Responsive Utilities**: `AdaptiveUtils` class with helper methods for responsive design calculations.
- **New `README.md`**: A new `README.md` has been created to provide a comprehensive overview of the package, its features, and how to use it.

### 🔄 Refactor

- **Updated Main Export**: The main `material_design.dart` file has been updated to export the new adaptive design system.
- **Code Cleanup**: Minor code cleanup and documentation improvements in `design_tokens.dart`.
