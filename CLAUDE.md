...
| `demo/` | Full Flutter web app — the live, interactive showcase deployed to GitHub Pages (https://fluttely.github.io/material_design/). | git, not pub |
...
- **demo/** — the live proof. Every token family and widget the package exports has a
  showcase page. New API ⇒ new (or extended) showcase page in the matching section
  (Expressive / Foundations / Styles / Utilities — see "Demo conventions").

Checklist for an API change: `lib/` + tests + README + `readme_showcase_test.dart` +
`example/lib/main.dart` + `demo/` page + `CHANGELOG.md` entry + (if concepts changed)
`documentation/material_design/` notes in both languages. Use `/new-scale` — it is
this checklist as a command.
...
- **Adding a page is one edit.** `demo/lib/showcase/showcase_destinations.dart` pairs
  every page with its destination and groups them into sections; the rail
...
- `demo/test/showcase_navigation_test.dart` renders every page in the list, so a page
  that throws on build fails CI rather than the deploy. It is written to be
  **order-independent**: it reads the selected destination off `showcaseDestinations`
  rather than naming it, scrolls the rail before tapping, and pumps bounded frames
  instead of `pumpAndSettle` (several pages animate forever). Reordering the rail must
  never fail a test about state layers — keep it that way.
